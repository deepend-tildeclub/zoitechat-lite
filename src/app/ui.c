...
static void on_character_chart(GtkButton *btn, gpointer user_data);
...
zc_ui_register_default_menu_items(void) {
  ...
  zc_ui_register_menu_action("Character Chart", G_CALLBACK(on_character_chart));
...
static GtkEntry *
zcl_current_entry(UiState *st) {
  if (!st || !st->notebook) return NULL;
  const gchar *target = current_target(st);
  if (!target) target = "status";
  ChatPage *page = g_hash_table_lookup(st->pages, target);
  if (!page && g_strcmp0(target, "status") != 0) {
    page = g_hash_table_lookup(st->pages, "status");
  }
  return page ? chat_page_get_entry(page) : NULL;
}

static gboolean
ascii_enter(GtkWidget *wid, GdkEventCrossing *event, GtkWidget *label) {
  (void)event;
  const char *text = gtk_button_get_label(GTK_BUTTON(wid));
  if (!text) return FALSE;
  gunichar u = g_utf8_get_char(text);
  char buf[64];
...
static void
ascii_click(GtkWidget *wid, gpointer user_data) {
  UiState *st = (UiState *)user_data;
  if (!st) return;
  const char *text = gtk_button_get_label(GTK_BUTTON(wid));
  if (!text) return;
  GtkEntry *entry = zcl_current_entry(st);
  if (!entry) return;
...
static void
ascii_open(UiState *st) {
  if (!st || !st->win) return;

  GtkWidget *win = gtk_window_new(GTK_WINDOW_TOPLEVEL);
  gtk_window_set_title(GTK_WINDOW(win), "Character Chart");
  gtk_window_set_transient_for(GTK_WINDOW(win), GTK_WINDOW(st->win));
  ...
  const unsigned char *table_pos = ascii_table;
  GtkWidget *hbox = NULL;
  while (table_pos[0] != 0) {
    if (table_pos[0] == '\n' || !hbox) {
      if (table_pos[0] == '\n') table_pos++;
    ...
    gint len = g_utf8_skip[table_pos[0]];
    char name[8];
    memcpy(name, table_pos, len);
    name[len] = '\0';

    GtkWidget *but = gtk_button_new_with_label(name);
    gtk_widget_set_size_request(but, 28, -1);
    g_signal_connect(but, "clicked", G_CALLBACK(ascii_click), st);
    g_signal_connect(but, "enter-notify-event", G_CALLBACK(ascii_enter), label);
...
static void
on_character_chart(GtkButton *btn, gpointer user_data) {
  (void)btn;
  UiState *st = (UiState *)user_data;
  if (!st || !st->win) return;
