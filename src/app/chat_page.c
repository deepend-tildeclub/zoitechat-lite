...
user_store_find(ChatPage *p, const gchar *nick, GtkTreeIter *out_iter) {
  ...
  gchar *needle = zcl_strip_ansi(nick);
  GtkTreeIter iter;
  ...
  while (valid) {
    ...
    gboolean match = (cur && needle && g_ascii_strcasecmp(cur, needle) == 0);
    g_free(cur);
    if (match) {
      g_free(needle);
...
chat_page_new(const gchar *target) {
  ...
  if (is_chan) {
    p->user_store = gtk_list_store_new(ZC_USERLIST_N_COLS, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING);
    ...
    GtkTreeViewColumn *c = gtk_tree_view_column_new_with_attributes("Users", r, "markup", ZC_USERLIST_COL_MARKUP, NULL);
...
chat_page_userlist_upsert(ChatPage *p, const gchar *nick, gchar prefix) {
  ...
  gchar *raw_disp = px ? g_strdup_printf("%c%s", px, nick) : g_strdup(nick);
  gchar *plain_disp = zcl_strip_ansi(raw_disp);
  gchar *markup = zcl_ansi_to_markup(raw_disp);
  gchar *plain_nick = zcl_strip_ansi(nick);
  gchar *fold = g_ascii_strdown(plain_nick, -1);
  gchar *sort = g_strdup_printf("%d %s", prefix_rank(px), fold);
  g_free(fold);

  GtkTreeIter iter;
  if (user_store_find(p, plain_nick, &iter)) {
    gtk_list_store_set(p->user_store, &iter,
      ZC_USERLIST_COL_NICK, plain_nick,
      ZC_USERLIST_COL_DISPLAY, plain_disp,
      ZC_USERLIST_COL_MARKUP, markup,
...
chat_page_append(ChatPage *p, const gchar *line) {
  ...
  zcl_buffer_insert_ansi(p->buffer, &end, line);
